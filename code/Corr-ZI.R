## Corr-ZI
# Title: Microbiome association analysis methods and strategies in zero-inflated microbiome data
# Version: 0.0.1
...
Univariate_CorrZI <- function(data, x, y, winsor = TRUE, winsor.qt = c(0,0.97), rep = 200){
...
CorrZI <- function(data, x, y, winsor = TRUE, winsor.qt = c(0,0.97), rep = 200,n_cores = 1){
# Initializes the parallel environment:
  myCluster = parallel::makeCluster(n_cores)
  doParallel::registerDoParallel(myCluster)
  
  res <- foreach(y = c(1:length(y)),.combine='rbind') %dopar% CorrZI()
